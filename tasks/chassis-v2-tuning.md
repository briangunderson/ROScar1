...
| 4 | Validate D435i depth voxel layer in costmap | **Done 2026-04-28**: with a real obstacle in front of the robot after a clean `sudo systemctl restart roscar-web` and nav-mode startup, the voxel layer marked 481–558 lethal cells in the local costmap while the lidar obstacle layer held steady at ~70. Depth-only obstacle detection contributes 7× more cells than the lidar layer for an in-FOV object. Known fragility: the voxel cell count drops to 0 if odom has drifted significantly before the costmap warms up — symptom is "Sensor origin out of map bounds — cannot raytrace" warnings. Workaround: clean roscar-web restart. See open items #12 (pickup detection) and #13 (local_costmap recenter race) below. |
| 5 | Dashboard "click map → set initial pose" | **Done 2026-04-28**: INIT button + Shift+click in [aio-map.js]. Click sets position with current TF yaw; click+drag sets explicit yaw from drag direction. Tighter covariance (10cm/15°) for drag, looser (25cm/30°) for plain click. Green arrow visualizes drag direction. |
| 6 | End-to-end landmark localizer test (ArUco markers in a closed loop) | **Prerequisite bug fixed 2026-04-28**: marker storage was a single global file, not per-map → reusing markers across different SLAM sessions corrupted nav. Fixed via per-map sidecar files (see "Per-map marker storage" below). Actual closed-loop test still pending. |
| 14 | Per-map marker storage (was a critical bug — ID 6 prerequisite) | **Done 2026-04-28**. Markers now stored in `<map_path>.markers.yaml` sidecar. `navigation.launch.py` derives the sidecar from the `map` arg and passes it as `learned_markers_file` to landmark_localizer. `launch_manager` `save_map` callback snapshots the live `~/roscar_ws/learned_markers.yaml` into the sidecar (or writes empty if no markers learned). `landmark_localizer` truncates the global file at slam-session startup so leftover markers from a previous map don't get shipped with a newly-saved one. |
